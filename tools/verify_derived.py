"""Verify an explicit Cable derived release without weakening the original baseline gate."""
import argparse
import hashlib
import json
import re
...
CARTRIDGE = PREFIX + 'render-readiness.js'
SOURCE = 'src/cable-geometry/render-readiness.js'
...
def verify(repo):
    ...
    try:
        ...
        require(m['applications'] == [{'id': 'cable-geometry-visualiser', 'entry': ENTRY}], 'derived release must own Cable only')
        ...
        require(set(listed) == set(originals) | {CARTRIDGE}, 'derived closure differs from baseline Cable plus cartridge')
